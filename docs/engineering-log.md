...
## 2026-09-20 — 人工复核补充与测量纠正

- 9bbf4ee真实轮次自动LIVE-01/02/09 PASS，03/08 FAIL。主agent截图复核发现“全部=0”与明细两条不一致，HTML用all而JS统计用全部；追加manual-review.json将LIVE-02判为FAIL并保留原始报告。补充总数断言和通用HTML/JS映射一致性生成指引，不手改模型产物。
- 该轮深色渐变实际已经覆盖整个画布，旧DOM颜色采样无法解释渐变；改为实际截图像素比例，继续使用原有亮度与75%覆盖要求。记账统计位于“收支汇总”region，测试未匹配而混入明细标签；据实际界面补充同义语义及明细排除。
- 手机tab隔离390px诊断确认selected类与可见区域始终一致，150ms背景过渡导致截图截到旧底色。截图禁用动画以记录稳定状态，未修改产品状态逻辑。
- CI35472331715完整工作台8失败/7通过，15数据库集成通过。独立Chromium复现inert期间heading可见、fill返回但字段仍空；测试saved()追加真实初始化完成等待。B-10实际后台时长、channel和调用次数断言保持原样，等待新CI复验。

## 2026-09-20 — 输出预算与当前失败证据

- CI35473020018 / 9f48138完整通过：104单元、73SQL断言、15数据库集成、19预览、15离线回放、18客户端、15工作台，以及lint/typecheck/build。初始化门槛等待的修复已完成完整复验，模型仍明确为fixture。
...
## 2026-09-21：计算器与完整生命周期技术复核启动

用户要求修复表达式求值，并补测同项目两轮增量、整个浏览器退出重开、恢复源码与Preview一致、恢复后修改基础版本、独立HTML及数据隔离。用户确认身份范围为同一浏览器保留身份关闭重开；不扩展跨设备登录。检查发现现有CSP不允许eval/Function动态执行，而旧静态检查未诊断该路径，具体评审源码及真实浏览器复现仍需核对。本轮新增docs/remediation.md逐项记录R-01～05，未跑不记PASS，历史证据不冒充新结果。
- 后续定向证据纠正初步假设：存量计算器v1/v2/v3均未使用eval/Function；pending.left被formatNumber转为string，applyOp加法直接拼接，Chromium回放三版本12+3均为123。因此当前根因是生成代码的数值类型错误，CSP检查仅为额外兼容性防错。生成协议增加数值/显示分离及四则自检要求，静态检查拒绝常见动态求值引用但不声称穷尽JavaScript安全分析。127单元、lint、typecheck通过；本轮真实模型完整业务验收仍未完成。

## 2026-09-21 技术复核完成

本地5a8fdef与生产11521ae分别完成同项目R-01～05真实模型断点续验，共16次调用、229528 tokens。覆盖评审精确按键、两轮增量、浏览器进程关闭重开、源码/Preview恢复、恢复后base与独立导出隔离。保留所有定位/等待失败及导出快速点击BRIDGE_RATE_LIMIT证据，正常导出每键100ms。生产原项目不会被部署自动改写。完整CI35596716095通过，最终测试适配lint/typecheck通过；主agent已核验加法同屏与恢复/导出截图。详见docs/remediation.md和两组脱敏summary.json。
