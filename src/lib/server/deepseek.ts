...
export async function callModel(name:'plan_app'|'write_app',messages:ModelMessage[],signal:AbortSignal):Promise<ModelResult>{
  ...
  try{response=await fetch(c.DEEPSEEK_BASE_URL.replace(/\/+$/,'')+'/chat/completions',{method:'POST',redirect:'error',signal,headers:{Authorization:`Bearer ${c.DEEPSEEK_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:c.DEEPSEEK_MODEL,thinking:{type:'disabled'},stream:false,temperature:0.3,max_tokens:name==='plan_app'?1024:16384,messages,tools:[{type:'function',function:{name,description:name==='plan_app'?'整理完整应用需求和本轮变更，不声称功能已经验证。':'提供完整 HTML、CSS 和异步 JS 函数体，等待宿主校验。',parameters}}],tool_choice:{type:'function',function:{name}}})});}
