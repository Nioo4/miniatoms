...
const counters = { starts: 0, runGets: 0, cancels: 0 };
const listeners: ((event: string, session: { user: { id: string } }) => void)[] = [];
const session = { user: { id: ownerId }, access_token: "fixture-only" };

declare global {
  interface Window {
    __clientFixture: { counters: typeof counters; changeIdentity(): void; mounts: string[] };
    __clientFixtureAuth: {
...
window.__clientFixture = { counters, mounts: [], changeIdentity() { for (const callback of listeners) callback("SIGNED_IN", { user: { id: "30000000-0000-4000-8000-000000000002" } }); } };
window.__clientFixtureAuth = {
  initializeSession: async () => session, accessToken: async () => session.access_token, refreshSession: async () => session,
...
async function start() {
  ...
  createRoot(document.getElementById("root")!).render(<Workbench projectId={projectId} />);
}
void start();
